...
SDL_AppResult SDL_AppInit(void** appstate, int argc, char* argv[])
{
    ...
    AppState* as = (AppState*)SDL_calloc(1, sizeof(AppState)); //fix by casting void* to AppState*
...
SDL_AppResult SDL_AppEvent(void* appstate, SDL_Event* event)
{
    ...
    case SDL_EVENT_KEY_DOWN: {
        ...
        if (index >= 0) {
            if (sym == SDLK_Z) players[index].wasd |= 1;
            if (sym == SDLK_Q) players[index].wasd |= 2;
    ...
    case SDL_EVENT_KEY_UP: {
        ...
        int index = whoseKeyboard(id, players, player_count);
        if (index >= 0) {
            if (sym == SDLK_Z) players[index].wasd &= 30;
            if (sym == SDLK_Q) players[index].wasd &= 29;
